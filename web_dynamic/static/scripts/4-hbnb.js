...
$(document).ready(() => {
  ...
  // Modified version from 3-hbnb.js
  const searchPlacesByAmenities = () => {
    $.ajax({
      url: 'http://localhost:5001/api/v1/places_search/',
      type: 'POST',
      contentType: 'application/json',
      data: JSON.stringify({ amenities: Object.keys(amenities) }), // Send only amenity IDs
      success: function (data) {
        console.log('Received places data:', data); // Debug: Log received data
        $('.places').empty(); // Clear out existing places in the page
  ...
  // Initial search with no amenities checked ?
  searchPlacesByAmenities();

  // Click event on button tag to trigger a search with currently checked amenities
  $('button').click(() => {
    searchPlacesByAmenities();
  });
});
